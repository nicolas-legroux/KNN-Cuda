...
__global__ void oddeven_step_multiple(double *distances, int* indexes,
		int n_train, int n_test, int step) {
	...
	if (step % 2 == 0 && id_thread_x % 2 == 0 && (id_thread_x + 1 < n_train)) {
		double data_left = distances[id_array_to_be_sorted * n_train
				+ id_thread_x];
		double data_right = distances[id_array_to_be_sorted * n_train
				+ id_thread_x + 1];

		if (data_left > data_right) {
			int index_left = indexes[id_array_to_be_sorted * n_train
					+ id_thread_x];
			distances[id_array_to_be_sorted * n_train + id_thread_x] =
					data_right;
			distances[id_array_to_be_sorted * n_train + id_thread_x + 1] =
					data_left;
			indexes[id_array_to_be_sorted * n_train + id_thread_x] =
					indexes[id_array_to_be_sorted * n_train + id_thread_x + 1];
			indexes[id_array_to_be_sorted * n_train + id_thread_x + 1] =
					index_left;
		}
	}

	else if (step % 2 == 1 && id_thread_x % 2 == 1
			&& (id_thread_x + 1 < n_train)) {
		double data_left = distances[id_array_to_be_sorted * n_train
				+ id_thread_x];
		double data_right = distances[id_array_to_be_sorted * n_train
				+ id_thread_x + 1];
...
void oddeven_sort_indexes_multiple(double *distances, int * indexes,
	...
	CudaSafeCall( cudaMalloc((void**)&d_distances, data_size));
	CudaSafeCall(
			cudaMemcpy(d_distances, distances, data_size, cudaMemcpyHostToDevice));

	CudaSafeCall(cudaMalloc((void**)&d_indexes, indexes_size));
	CudaSafeCall(
			cudaMemcpy(d_indexes, indexes, indexes_size, cudaMemcpyHostToDevice));
